...
class coordinatorDashboard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final cardsPerRow = (availableWidth / (minCardWidth + spacing)).floor().clamp(1, 4);
    final cardWidth = (availableWidth - (spacing * (cardsPerRow - 1))) / cardsPerRow;

    return BlocBuilder<commonBloc, commonState>(
      builder: (context, state) {
        final dashboardBloc = context.read<commonBloc>();
        if (state is commonLoading) {
          ...
          final inProgressTasks = state.tasks.where((t) => t.Status == 'In Progress').length;
          final pendingTasks = state.tasks.where((t) => t.Status == 'Pending').length;
          final unverifiedTasks = state.tasks.where((t) => t.Status == 'In Verification').length;
          final activeVolunteers = state.users.where((u) => u.inTask).length;
          final pendingRequests = state.requests.where((r) => r.status == 'Pending').length;

          final Fooditems = state.resources.where((r) => r.ResourceType == 'Food').length;
          final Wateritems = state.resources.where((r) => r.ResourceType == 'Water').length;
          final Medicalitems = state.resources.where((r) => r.ResourceType == 'Medicine').length;
          final Shelteritems = state.resources.where((r) => r.ResourceType == 'Shelter').length;

          final taskStatusData = [
            {'label': 'Completed', 'value': completedTasks.toDouble(), 'color': successColor},
            {'label': 'In Progress', 'value': inProgressTasks.toDouble(), 'color': accentColor},
            {'label': 'In Verification.', 'value': unverifiedTasks.toDouble(), 'color': warningColor},
            {'label': 'Pending', 'value': pendingTasks.toDouble(), 'color': infoColor},
          ];
          ...
          return SingleChildScrollView(
            ...
            child: Column(
              ...
              children: [
                ...
                ResourceRequestBarChart(data: resourceRequestData, unitLabel: 'Units Requested'),
                ...
                DashboardCard(
                  ...
                  onTap: () => Navigator.push(
                    ...
                    MaterialPageRoute(
                      builder: (context) =>
                          BlocProvider(create: (context) => CoordinatorBloc(), child: ViewHelpersPage()),
                    ),
                  ),
                ),
                const SizedBox(height: 12),
                DashboardCard(
                  ...
                  onTap: () async {
                    final bool res = await Navigator.push(
                      ...
                      MaterialPageRoute(
                        builder: (context) =>
                            BlocProvider(create: (context) => CoordinatorBloc(), child: VerifyTasksPage()),
                      ),
                    );
                    if (res == true) {
                ...
                DashboardCard(
                  ...
                  onTap: () => Navigator.push(
                    ...
                    MaterialPageRoute(
                      builder: (context) =>
                          BlocProvider(create: (context) => CoordinatorBloc(), child: ViewResourcesPage()),
                    ),
                  ),
                ),
                const SizedBox(height: 12),
                DashboardCard(
                  ...
                  onTap: () => Navigator.push(
                    ...
                    MaterialPageRoute(
                      builder: (context) =>
                          BlocProvider(create: (context) => CoordinatorBloc(), child: ResourceResponsesPage()),
                    ),
                  ),
                ),
                const SizedBox(height: 16),
